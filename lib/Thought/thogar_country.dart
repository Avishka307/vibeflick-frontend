...
import '../sample/location_cache_service.dart';

class ThogarCountryScreen extends StatefulWidget {
  ...
  @override
  State<ThogarCountryScreen> createState() => _ThogarCountryScreenState();
}

class _ThogarCountryScreenState extends State<ThogarCountryScreen> {
  ...
  String get _flagEmoji => _countryFlagFromCode(widget.countryCode);

  @override
  void initState() {
    super.initState();
    _fetchCountryPosts();
    // ✅ Background cache sync — next time faster
    _syncCacheIfNeeded();
  }

  // ── NEW: Internet check ────────────────────────────────────────────────────
  Future<bool> _checkInternetCountry() async {
    try {
  ...
  Future<void> _fetchCountryPosts() async {
    // ── NEW: internet check ──────────────────────────────────────────────────
    if (!await _checkInternetCountry()) return;
    ...
    try {
      ...
      debugPrint('\n🌍 ========== FETCHING COUNTRY POSTS ==========');
      debugPrint('   Country : ${widget.countryName} (${widget.countryCode})');
      debugPrint('   URL     : $uri');
      ...
      if (response.statusCode == 200 && body['success'] == true) {
        ...
        setState(() {
          _posts = data.map((e) => _CountryPost.fromJson(e)).toList();
          _loading = false;
        });
  ...
  void _openComments(_CountryPost post) async {
  ...
  void _openLike(_CountryPost post) {
    final user = FirebaseAuth.instance.currentUser;
    if (user == null) return;
    ThogarLike.showLikedBySheet(context, post.id, post.likeCount);
  }

  void _openShare(_CountryPost post) {
  ...
  void _openRepost(_CountryPost post) async {
    final user = FirebaseAuth.instance.currentUser;
    if (user == null) return;
      ...
      body: CustomScrollView(slivers: [
        ...
        SliverAppBar(
          ...
          title: Row(children: [
            Container(
              ...
              child: Text(
                _flagEmoji.isNotEmpty ? _flagEmoji : '🌍',
            ...
            Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Text(widget.countryName,
                  style: const TextStyle(color: Colors.white,
                      ...
                      letterSpacing: 0.3)),
              Text('${_posts.length} thoughts nationwide',
        ...
        SliverToBoxAdapter(
          child: Padding(
            ...
            child: Row(children: [
              ...
              const Text('Sorted by most recent',
                  style: TextStyle(color: Colors.amber,
                      fontSize: 11, fontWeight: FontWeight.w600,
                      letterSpacing: 0.5)),
              const SizedBox(width: 8),
              Expanded(child: Container(
                  height: 1, color: Colors.amber.withOpacity(0.2))),
            ]),
          ),
        ),
        ...
        if (_loading)
          ...
          if (_errorMessage != null)
            SliverFillRemaining(
              child: _ErrorState(
                  message: _errorMessage!, onRetry: _fetchCountryPosts),
            )
          else
            if (_posts.isEmpty)
              SliverFillRemaining(
                child: _EmptyState(
                  icon: _flagEmoji.isNotEmpty ? _flagEmoji : '🌍',
                  title: 'No thoughts yet in ${widget.countryName}',
                  subtitle: 'Be the first to post nationwide!',
                ),
              )
            else
              SliverList(
  ...
  Future<void> _syncCacheIfNeeded() async {
    final cache = LocationCacheService.instance;
    if (!cache.hasCachedLocation) {
...
//  Country Post Card
// ─────────────────────────────────────────────────────────────────────────────
class _CountryPostCard extends StatefulWidget {
  final _CountryPost   post;
  ...
  @override
  State<_CountryPostCard> createState() => _CountryPostCardState();
}

class _CountryPostCardState extends State<_CountryPostCard> {
  static const String _baseUrl = 'https://avishka-tiktok-api.zeabur.app';
            ...
            child: Column(
              ...
              children: [
                ...
                Text(widget.post.content,
                    style: const TextStyle(color: Colors.white,
                        fontSize: 18, height: 1.45, fontWeight: FontWeight.w500)),
                ...
                Row(
                  ...
                  children: [
                    ...
                    GestureDetector(onTap: widget.onShare,
                        child: const _AChip(icon: Icons.ios_share_rounded, label: '')),
                  ],
                ),
              ],
            ),
...
class _AChip extends StatelessWidget {
  final IconData icon;
  final String   label;
  const _AChip({required this.icon, required this.label});
  @override
...
class _CountryPost {
  ...
  factory _CountryPost.fromJson(Map<String, dynamic> json) {
